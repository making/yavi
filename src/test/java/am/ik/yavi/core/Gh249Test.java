...
public class Gh249Test {
	...
	void failFastOfNestedValidator() {
		...
		final List<String> violations = aValidator.validate(a)
			.violations()
			.stream()
	...
	void failFastOfConditionalValidator() {
		final Validator<A> validator = ValidatorBuilder.of(A.class)
			.constraintOnGroup(Group.UPDATE, builder -> builder.nest(A::b, "b", bValidator).failFast(true))
			.constraintOnGroup(Group.UPDATE, builder -> builder.forEach(A::bs, "bs",
					b -> b.constraint(B::x, "x", c -> c.greaterThan(0)).constraint(B::y, "y", c -> c.equalTo("y")))) //
			.build();
		final List<String> violations = validator
			.validate(new A(invalidB, Collections.singletonList(invalidB)), Group.UPDATE)
			.violations()
			.stream()
			.map(ConstraintViolation::messageKey)
			.collect(toList());
