...
class CollectionValidatorTest extends AbstractCollectionValidatorTest {
	...
	void nullCollectionValid() throws Exception {
		Validator<FormWithCollection> validator = ValidatorBuilder.of(FormWithCollection.class) //
			.forEachIfPresent(FormWithCollection::getAddresses, "addresses", addressValidator)
			.build();
	...
	void stringListAllInValid() throws Exception {
		...
		Validator<String> stringValidator = ValidatorBuilder.of(String.class)
			.constraint(String::toString, "value", c -> c.notNull().lessThanOrEqual(2))
			.build();
		...
		assertThat(violations.get(0).message())
			.isEqualTo("The size of \"texts[0].value\" must be less than or equal to 2. The given size is 3");
		assertThat(violations.get(0).messageKey()).isEqualTo("container.lessThanOrEqual");
		assertThat(violations.get(1).message())
			.isEqualTo("The size of \"texts[1].value\" must be less than or equal to 2. The given size is 3");
		assertThat(violations.get(1).messageKey()).isEqualTo("container.lessThanOrEqual");
		assertThat(violations.get(2).message())
			.isEqualTo("The size of \"texts[2].value\" must be less than or equal to 2. The given size is 3");
	...
	void stringListAllInValidEmptyNestedName() throws Exception {
		...
		assertThat(violations.size()).isEqualTo(3);
		assertThat(violations.get(0).message())
			.isEqualTo("The size of \"texts[0]\" must be less than or equal to 2. The given size is 3");
		assertThat(violations.get(0).messageKey()).isEqualTo("container.lessThanOrEqual");
		assertThat(violations.get(1).message())
			.isEqualTo("The size of \"texts[1]\" must be less than or equal to 2. The given size is 3");
		assertThat(violations.get(1).messageKey()).isEqualTo("container.lessThanOrEqual");
		assertThat(violations.get(2).message())
			.isEqualTo("The size of \"texts[2]\" must be less than or equal to 2. The given size is 3");
		assertThat(violations.get(2).messageKey()).isEqualTo("container.lessThanOrEqual");
	}
